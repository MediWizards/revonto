...
def test_enforce_data_structure():
    ...
    with pytest.raises(
        ValueError,
        match=re.escape(
            "All elements in the set for key GO:1234 must be Annotation objects."
        ),
    ):
        annodict["GO:1234"] = {"A"}
    with pytest.raises(
        ValueError,
        match=re.escape("Value for key GO:1234 must be a set of Annotation objects."),
    ):
        annodict["GO:1234"] = anno1
    with pytest.raises(
        ValueError,
        match=re.escape(
            "All Annotation objects must have the same term_id as the key (GO:5678)."
        ),
    ):
...
def test_add_two_anotations_dicts():
    ...
    assert (
        annodict1 + annodict2 == annodict2 + annodict1
    )  # addition should be the same regardles of order

    annodict12 = annodict1 + annodict2
    assert len(annodict12) and "GO:1234" in annodict12  # only one key should exist
    assert (
        len(annodict12["GO:1234"]) == 2
    )  # two Annotation s should be present under this key

    annodict11 = annodict1 + annodict1
    assert (
        len(annodict11) == 1 and len(annodict11["GO:1234"]) == 1
    )  # adding same Annotation to the set should not change it
