...
def test_fisher_example():
    assert pytest.approx(
        fisherobj.pval_obj.calc_pvalue(2, 12, 16, 29),
        rel=1e-6) == 0.0007206922 #round the fisher exact score to a reasonable amount
...
def test_binomial_example():
    assert pytest.approx(
        binomialobj.pval_obj.calc_pvalue(2,12,16,29),
        rel=1e-6) == 0.00841718
